...
export default function GateInPage() {
  ...
  const handleOpenGate = () => {
    setGateStatus("Open");
    setTimeout(() => setGateStatus("Closed"), 5000); // Auto-close after 5 seconds
  };
  ...
  const cameraLabel =
    cameraDevices.find((device) => device.deviceId === selectedDeviceId)
      ?.label || "Pilih kamera";
  ...
  const fetchLogs = async () => {
    try {
      ...
      const formatted = (data.logs || []).map((l: any) => {
        const dt = l.entry_time ? new Date(l.entry_time) : null;
        const timeStr = dt
    ...
    const connect = () => {
      ...
      ws.onmessage = (evt) => {
        try {
          ...
          // Only show toast for new parked-in entries (VEHICLE_ENTRY)
          // and only when the vehicle_type matches the current URL param
          if (payload?.event === "VEHICLE_ENTRY") {
            const incomingType = payload.vehicle_type ?? "";
            // If vehicleType is falsy or matches, show toast; otherwise suppress
            ...
            toast.info("Kendaraan Masuk", {
              description: plate
                ? `${plate} berhasil masuk ke area parkir.`
      ...
      <div className="flex md:flex-row justify-center items-center md:items-center mb-7.5">
        <h1 className="text-[48px] text-text-color flex items-center">
          ...
          <span className="text-secondary-color">
            In ({vehicleType === "motorbike" ? "Motorbike" : "Car"})
          </span>
        </h1>
      </div>
      ...
      <div className="grid grid-cols-7 grid-rows-8 gap-7.5 w-full flex-1 min-h-0">
        ...
        <div className="col-span-2 row-span-6 h-full min-h-0">
          <Card className="w-full h-full p-5">
            <CardHeader>
              <CardTitle className="flex items-center gap-5">
                <h1 className="tracking-[5%]">Gate-In log</h1>
              </CardTitle>
            </CardHeader>
            <hr />
            <CardContent>
